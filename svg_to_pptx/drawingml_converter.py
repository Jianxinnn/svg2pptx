...
def collect_class_styles(root: ET.Element) -> dict[str, dict[str, str]]:
    """Collect simple `.class { key: value; }` CSS rules from SVG <style> tags."""
    class_styles: dict[str, dict[str, str]] = {}

    def _parse_css(css_text: str) -> None:
        # Strip block comments for safer regex parsing.
        cleaned = re.sub(r'/\*.*?\*/', '', css_text, flags=re.S)
        for selector, body in re.findall(r'([^{}]+)\{([^{}]+)\}', cleaned):
            props: dict[str, str] = {}
            ...
            for token in selector.split(','):
                ...
                if token.startswith('.') and len(token) > 1:
                    class_styles[token[1:]] = dict(props)

    for style_elem in root.iter(f'{{{SVG_NS}}}style'):
        if style_elem.text:
            _parse_css(style_elem.text)
