...
def _make_circle_shape(
    ...
    ctx: ConvertContext,
    fill_xml: str,
    stroke_xml: str = '<a:ln><a:noFill/></a:ln>',
    name_prefix: str = 'Ellipse',
) -> str:
    """Build a simple ellipse shape centered at (cx, cy)."""
    x = cx - r
    y = cy - r
    d = r * 2
    ...
    return _wrap_shape(
        shape_id, f'{name_prefix} {shape_id}',
        px_to_emu(x), px_to_emu(y), px_to_emu(d), px_to_emu(d),
        geom, fill_xml, stroke_xml,
    )


def _pattern_circle_fill_group(
    elem: ET.Element,
    ctx: ConvertContext,
    pattern_id: str,
) -> str:
    """Expand a simple circle pattern fill on a rect into tiled circle shapes."""
    pattern = ctx.defs.get(pattern_id)
    if pattern is None or pattern.tag.replace(f'{{{SVG_NS}}}', '') != 'pattern':
        return ''

    circle_child = None
    for child in pattern:
        if child.tag.replace(f'{{{SVG_NS}}}', '') == 'circle':
            circle_child = child
            break
    if circle_child is None:
        return ''

    tile_w = _f(pattern.get('width'))
    tile_h = _f(pattern.get('height'))
    ...
    base_cx = _f(circle_child.get('cx'))
    base_cy = _f(circle_child.get('cy'))
    base_r = _f(circle_child.get('r'))
    if base_r <= 0:
        return ''

    fill_op = get_fill_opacity(circle_child, ctx)
    fill = build_fill_xml(circle_child, ctx, fill_op)

    cols = max(1, int(math.ceil(w / tile_w)))
    rows = max(1, int(math.ceil(h / tile_h)))

    shapes: list[str] = []
    for row in range(rows):
        for col in range(cols):
            cx_pos = x + col * tile_w + base_cx
            cy_pos = y + row * tile_h + base_cy
            if cx_pos + base_r < x or cx_pos - base_r > x + w:
                continue
            if cy_pos + base_r < y or cy_pos - base_r > y + h:
                continue
            shapes.append(
                _make_circle_shape(
                    cx_pos,
    ...
    min_x = px_to_emu(x)
    min_y = px_to_emu(y)
    group_w = max(px_to_emu(w), 1)
    group_h = max(px_to_emu(h), 1)
    group_id = ctx.next_id()
    return f'''<p:grpSp>
<p:nvGrpSpPr>
<p:cNvPr id="{group_id}" name="Pattern Fill {group_id}"/>
<p:cNvGrpSpPr/>
<p:nvPr/>
</p:nvGrpSpPr>
<p:grpSpPr>
...
def _extract_round_cap_dots(
    commands: list[PathCommand],
    stroke_width: float,
) -> list[tuple[float, float, float]]:
    """Extract dot centers from zero-length round-cap path segments."""
    if stroke_width <= 0:
        return []
    ...
    for cmd in commands:
        ...
        if cmd.cmd == 'L':
            ...
            if abs(x2 - current_x) < 1e-6 and abs(y2 - current_y) < 1e-6:
                dots.append((x2, y2, stroke_width / 2.0))
...
def convert_path(elem: ET.Element, ctx: ConvertContext) -> str:
    ...
    stroke_width = _f(_get_attr(elem, 'stroke-width', ctx), 0)
    dot_points = _extract_round_cap_dots(commands, stroke_width) if linecap == 'round' else []
    if dot_points:
        stroke_op = get_stroke_opacity(elem, ctx)
        stroke_color = _get_attr(elem, 'stroke', ctx)
        grad_id = resolve_url_id(stroke_color) if stroke_color else None
        if grad_id and grad_id in ctx.defs:
            dot_fill = build_gradient_fill(ctx.defs[grad_id], stroke_op)
        else:
            color = parse_hex_color(stroke_color) if stroke_color else None
            dot_fill = build_solid_fill(color, stroke_op) if color else '<a:noFill/>'
